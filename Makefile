...
help:
	...
	@echo "  deploy-fastapi          - Deploy FastAPI ANPR application"
	@echo "  create-networks        - Create Swarm networks"
	@echo ""
	@echo "Deployment Removal:"
	@echo "  remove-fastapi         - Remove FastAPI ANPR deployment"
	...
	@echo ""
	@echo "SSH Management:"
	@echo "  ssh-agent             - Start SSH agent and add keys"
	@echo "  ssh_fix               - Fix SSH host key issues"
	@echo ""
...
ssh_fix:
	@echo "Fixing SSH host key issues for KVM VMs..."
	@echo "Getting VM IPs from inventory..."
	@echo "Processing docker_swarm1..."
	@vm_ip1=$$(grep -A 5 'docker_swarm1:' inventory/created_vms.yml | grep 'ansible_host:' | cut -d: -f2 | tr -d ' '); \
	if [ -n "$$vm_ip1" ]; then \
		echo "Removing old host key for $$vm_ip1..."; \
		ssh-keygen -f ~/.ssh/known_hosts -R "$$vm_ip1" 2>/dev/null || true; \
		echo "Testing SSH connection to $$vm_ip1..."; \
		if ssh -o StrictHostKeyChecking=no ubuntu@$$vm_ip1 "echo 'SSH connection successful'" >/dev/null 2>&1; then \
			echo "✅ SSH connection to $$vm_ip1 is working"; \
			echo "Adding GitHub host key to VM $$vm_ip1..."; \
			ssh -o StrictHostKeyChecking=no ubuntu@$$vm_ip1 "ssh-keyscan -H github.com >> ~/.ssh/known_hosts" 2>/dev/null || true; \
			echo "✅ GitHub host key added to $$vm_ip1"; \
		else \
			echo "❌ Cannot connect to VM docker_swarm1 at $$vm_ip1"; \
		fi; \
	else \
		echo "❌ Could not get IP for docker_swarm1"; \
	fi; \
	echo ""; \
	echo "Processing docker_swarm2..."; \
	@vm_ip2=$$(grep -A 5 'docker_swarm2:' inventory/created_vms.yml | grep 'ansible_host:' | cut -d: -f2 | tr -d ' '); \
	if [ -n "$$vm_ip2" ]; then \
		echo "Removing old host key for $$vm_ip2..."; \
		ssh-keygen -f ~/.ssh/known_hosts -R "$$vm_ip2" 2>/dev/null || true; \
		echo "Testing SSH connection to $$vm_ip2..."; \
		if ssh -o StrictHostKeyChecking=no ubuntu@$$vm_ip2 "echo 'SSH connection successful'" >/dev/null 2>&1; then \
			echo "✅ SSH connection to $$vm_ip2 is working"; \
			echo "Adding GitHub host key to VM $$vm_ip2..."; \
			ssh -o StrictHostKeyChecking=no ubuntu@$$vm_ip2 "ssh-keyscan -H github.com >> ~/.ssh/known_hosts" 2>/dev/null || true; \
			echo "✅ GitHub host key added to $$vm_ip2"; \
		else \
			echo "❌ Cannot connect to VM docker_swarm2 at $$vm_ip2"; \
		fi; \
	else \
		echo "❌ Could not get IP for docker_swarm2"; \
	fi; \
	echo ""; \
	echo "SSH host key fix completed for all VMs!"
